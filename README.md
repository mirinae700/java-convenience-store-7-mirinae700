...
### 재고
- 상품별 관리
  - 상품별 이름, 가격, 수량, 프로모션 여부를 관리한다.
  - 관리항목 : `name`, `price`, `quantity`, `promotion`
- 상품 입고
  - 상품목록 파일을 통해 상품을 입고한다.
- 상품 출고
  - 사용자가 요청한 상품의 수량을 차감한다.

### 할인
- 할인정보
  - 프로모션 및 멤버십 할인과 관련된 정보를 관리한다.
  - 관리항목 : `name`, `buy`, `get`, `start_date`, `end_date`
...
- 장바구니에 상품을 담는다.
  - 프로모션 상품 확인
    - 증정 상품 확인 후 증정 상품 목록에 추가한다.
    - 부족한 증정 상품(재고는 있으나 요청한 수량이 적은 경우) 수량을 안내한다.
    - 재고 부족 시 프로모션 지적용 수량을 안내한다.
- 금액 계산
  - 총 구매 금액을 계산한다.
  - 행사 할인 금액을 계산한다.
  - 멤버십 할인 금액을 계산한다. (프로모션 미적용 금액에 한하여 30%, 최대 8,000원 할인)
  - 최종 결제 금액을 계산한다.
